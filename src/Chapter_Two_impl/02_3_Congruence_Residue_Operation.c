
#include "../../inc/Chapter_Two/02_3_Congruence_Residue_Operation.h"

/********************************************************************************
* @author: Li jiaqi
* @email: 547222867@qq.com
* @date: 2022/9/16 20:13
* @version: 1.0
* @description:
********************************************************************************/


int residue_addition(int a, int b, int m) {
    return modular_addition(a,b,m);
}

int residue_multiplication(int a, int k, int m) {
    int i;
    int result = 0;
    for (i = 0; i < k; i++){
        result = residue_addition(a,result,m);
    }
    return result;
}

void testResidue_addition(int a, int b, int m) {
    printf("%d + %d (mod %d) = %d",a,b,m, residue_addition(a,b,m));

}
void testResidue_multiplication(int a, int k, int m) {
    printf("%d * %d (mod %d) = %d",a,k,m, residue_multiplication(a,k,m));
}
